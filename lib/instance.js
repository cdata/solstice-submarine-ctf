...
define(['game/object', 'game/arbiter', 'model/game/fork', 'model/game/instance', 'model/game/move', 'game/vector2'],
  ...
  var Instance = GameObject.extend({
    ...
    resolveMoves: function() {
      ...
      var subForkState = this.model.get('subFork').get('position');
      var rktForkState = this.model.get('rktFork').get('position');

      if (subForkState) {
        subForkState = subForkState.toString();
      } else {
        subForkState = '{ Carried }';
      }

      if (rktForkState) {
        rktForkState = rktForkState.toString();
      } else {
        rktForkState = '{ Carried }';
      }
      ...
      var subForkResult = this.model.get('subFork').get('position');
      var rktForkResult = this.model.get('rktFork').get('position');

      if (subForkResult) {
        subForkResult = subForkResult.toString();
      } else {
        subForkResult = '{ Carried }';
      }

      if (rktForkResult) {
        rktForkResult = rktForkResult.toString();
      } else {
        rktForkResult = '{ Carried }';
      }

      console.log('Outcomes resolved.',
                  'Sub Fork',
                  subForkState, '=>', subForkResult,
                  'Rkt Fork',
                  rktForkState, '=>', rktForkResult);
